...
#     )
#     store.add_documents([document])
#     store.save_local(os.getenv("FAISS_INDEX_PATH"), index_name="faiss")
#     return memory


# @tool
# def search_recall_memories(comb_str: str, config: RunnableConfig) -> List[Document]:
#     """Search and retrieve past saved email conversations or outputs relevant to a given query."""
#     user_id = get_user_id(config)
...
@tool
def Retrieve_data(query: str, email_sub: Optional[str] = None, config: Optional[RunnableConfig] = None) -> List[str]:
    """Retrieve relevant documents based on query and optional subject."""
    combined_str = f"""
     <email_subject>{email_sub}</email_subject>
     <query>{query}</query>"""
...
def composing_email_tool(email_details:dict[str,str], query:str, config:RunnableConfig) -> str:
    ...
    context = Retrieve_data.invoke({
        "query":query,
        "email_sub":email_details["subject"],
        "config":config
    })
